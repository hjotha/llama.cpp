...
bool llama_block_manager::has_free_gpu_blocks(uint32_t num_requested_blocks, bool ignore_watermark) const {
    size_t curr_free_gpus = free_gpu_ids.size();
    if (!ignore_watermark && curr_free_gpus < watermark_gpu_safety_num_blocks) {
        return false;
    }
    return ignore_watermark ? num_requested_blocks <= curr_free_gpus
                            : num_requested_blocks <= (curr_free_gpus - watermark_gpu_safety_num_blocks);
}
