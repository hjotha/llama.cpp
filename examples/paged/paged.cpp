...
static void add_request_from_pool(struct llama_paged_scheduler * scheduler,
    ...
    if (!params.prompt_file.empty()) {
        ...
        GGML_ASSERT(ifs.is_open() && "failed to open prompt file");
        std::stringstream ss;
        ss << ifs.rdbuf();
        input_prompt = ss.str();
    } else {
        input_prompt = k_prompts[pool_index % k_n_prompts];
    }
...
int main(int argc, char ** argv) {
    ...
    for (int i = 0; i < params.n_sequences; ++i) {
        add_request_from_pool(scheduler, ctx, params, (size_t) i, i);
    ...
    while (true) {
        ...
        for (int i = 0; i < info->n_seq; ++i) {
            ...
            bool stop = (!params.no_eos && llama_vocab_is_eog(vocab, next_token)) || state.n_decoded >= params.n_predict;
        ...
        n_check += (int32_t) sampled_tokens.size();
        if (params.n_checkpoint > 0 && (uint32_t) n_check >= params.n_checkpoint) {
            const int64_t t_now_us = ggml_time_us();
            const float   tps_win  = (t_now_us - t_check_us) / 1e6f > 0 ? n_check / ((t_now_us - t_check_us) / 1e6f) : 0.0f;
            LOG_INF("checkpoint: decoded=%d tps_window=%.2f\n", n_check, tps_win);
            t_check_us = t_now_us;
            n_check    = 0;
        }
    }
