...
void llama_kv_cache_paged::init(const std::vector<ggml_backend_t> & layer_backends,
    ...
    layer_pool_ids.assign(n_layers, -1);
    layer_pool_indices.assign(n_layers, -1);

    // Group attention layers by their actual model backend. Every pool gets
    // the same logical block range so one block table remains valid across
    // devices; only the physical tensor backing differs.
    for (uint32_t il = 0; il < n_layers; ++il) {
        ...
        auto it = std::find(gpu_backends.begin(), gpu_backends.end(), layer_backend);
        int32_t pool_id;
        if (it == gpu_backends.end()) {
            pool_id = gpu_backends.size();
            gpu_backends.push_back(layer_backend);
            kv_gpu_layers.emplace_back();
        } else {
            pool_id = it - gpu_backends.begin();
        }

        layer_pool_ids[il] = pool_id;
        layer_pool_indices[il] = kv_gpu_layers[pool_id].size();
        kv_gpu_layers[pool_id].push_back(nullptr);
        attn_layer_ids.push_back(il);
    }

    for (size_t pool_id = 0; pool_id < gpu_backends.size(); ++pool_id) {
        struct ggml_init_params gpu_params;
        gpu_params.mem_size   = ggml_tensor_overhead() * 5 * kv_gpu_layers[pool_id].size();
        gpu_params.mem_buffer = NULL;
        gpu_params.no_alloc   = true;

        ggml_context_ptr ctx_gpu { ggml_init(gpu_params) };
        GGML_ASSERT(ctx_gpu && "Failed to initialize GPU KV cache context");

        for (const uint32_t il : attn_layer_ids) {
            if (layer_pool_ids[il] != (int32_t) pool_id) {
                continue;
            }

            // Since GGML_MAX_DIMS is set to 4, flatten to:
            // [num_blocks, 2 * n_heads_kv, block_size, head_dim].
            kv_gpu_layers[pool_id][layer_pool_indices[il]] = ggml_new_tensor_4d(
                ctx_gpu.get(), type, head_dim, block_size, 2 * n_heads_kv, n_gpu_blocks);
        }

        ggml_backend_buffer_ptr buf_gpu { ggml_backend_alloc_ctx_tensors(ctx_gpu.get(), gpu_backends[pool_id]) };
        GGML_ASSERT(buf_gpu && "Failed to allocate GPU KV cache buffer");
        ggml_backend_buffer_clear(buf_gpu.get(), 0);
        for (const auto * tensor : kv_gpu_layers[pool_id]) {
            GGML_ASSERT(tensor && tensor->buffer && "GPU layer tensor has null buffer");
        }
        LLAMA_LOG_INFO(
            "%s: pool %zu device=%s attention_layers=%zu buffer=%8.2f MiB\n",
            __func__, pool_id,
            ggml_backend_dev_name(ggml_backend_get_device(gpu_backends[pool_id])),
            kv_gpu_layers[pool_id].size(),
            ggml_backend_buffer_get_size(buf_gpu.get()) / 1024.0 / 1024.0);
        gpu_ctxs_bufs.emplace_back(std::move(ctx_gpu), std::move(buf_gpu));
    }
...
bool llama_kv_cache_paged::allocate(int32_t num_tokens, llama_sequence_group & group) {
    ...
    if (!block_manager.has_free_gpu_blocks(num_requested_blocks, /*ignore_watermark=*/!group.block_table.empty())) {
...
void llama_kv_cache_paged::do_block_copy(const llama_block_ids & src_ids,
    ...
    for (uint32_t il = 0; il < attn_layer_ids.size(); ++il) {
        ...
        const int32_t pool_id = layer_pool_ids[layer_id];
        const int32_t pool_index = layer_pool_indices[layer_id];
        struct ggml_tensor * gpu_main = kv_gpu_layers[pool_id][pool_index];
        struct ggml_tensor * cpu_main = kv_cpu_layers[il];
...
bool llama_kv_cache_paged::swap_in(llama_sequence_group & group) {
    ...
    if (!block_manager.has_free_gpu_blocks(num_blocks, /*ignore_watermark=*/true)) {
        return false;
    }
...
struct ggml_tensor * llama_kv_cache_paged::get_kv_tensor(int layer_idx) const {
    GGML_ASSERT(layer_idx >= 0 && layer_idx < (int32_t) layer_pool_ids.size());
    GGML_ASSERT(layer_pool_ids[layer_idx] >= 0 && "layer not in paged KV cache");
    return kv_gpu_layers[layer_pool_ids[layer_idx]][layer_pool_indices[layer_idx]];
}
...
std::map<ggml_backend_buffer_type_t, size_t> llama_kv_cache_paged::memory_breakdown() const {
    ...
    for (const auto & pool : kv_gpu_layers) {
        for (const auto * kv_gpu : pool) {
